...
export class FlashcardService {
    ...
    async createFlashcard(user: any, createFlashcardDto: CreateFlashcardDto): Promise<Flashcard> {
        ...
        const flashcardList = await this.flashcardListService.findOne(createFlashcardDto.flashcardListId);
        if (!flashcardList) {
            throw new Error('FlashcardList not found');
        }

        // Đảm bảo rằng _id có kiểu Mongoose ObjectId
        const flashcardId = createdFlashcard._id instanceof Types.ObjectId ?
    ...
    async isFlashcardBelongsToUser(flashcardId: string, userId: string): Promise<boolean> {
    ...
    async deleteFlashcardAndRemoveFromList(flashcardId: string, flashcardListId: string, userId: string): Promise<void> {
        // Kiểm tra xem flashcard có thuộc về người dùng hay không
        const isBelongsToUser = await this.isFlashcardBelongsToUser(flashcardId, userId);
        if (!isBelongsToUser) {
        ...
        const flashcardList = await this.flashcardListService.findOne(flashcardListId);
        if (!flashcardList) {
            throw new NotFoundException('FlashcardList not found');
        }
        const flashcardIndex = flashcardList.flashcards.findIndex(flashcard => flashcard._id.toString() === flashcardId);
        if (flashcardIndex !== -1) {
