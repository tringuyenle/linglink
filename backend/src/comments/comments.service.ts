...
export class CommentsService {
  ...
  async createComment(user: User, createCommentDto: CreateCommentDTO): Promise<Comment> {
    ...
    const newComment = await (await (await createdComment.save()).populate('comment')).populate('post')
  ...
  async updateNumComments(comment: Comment, changedCommentCount: number) {
    try {
      ...
      if (thisComment.comment) {
        ...
        this.postsService.changeNumComments(thisComment.post, changedCommentCount)
  ...
  async getCommentsWithReactByPostId(
    postId: string,
    userId: string
  ): Promise<{ data: Comment; like: boolean; dislike: boolean; numlikes: number; numdislikes: number }[]> {
    ...
    const transformedComments = await Promise.all(
      comments.map(async (comment) => {
        ...
        const checkReactionStatus = userId
          ? await this.reactionsService.checkCommentReactionStatus(userId, comment._id.toString())
          : null
  ...
  async getCommentsByComment(comment: Comment): Promise<Comment[]> {
    return this.commentModel.find({ comment: comment._id.toString() }).populate('comment').populate('post').exec()
  }
  ...
  async updateCommentsById(user: User, commentId: string, updateCommentDto: UpdateCommentDTO): Promise<Comment> {
    const currentComment = await this.getCommentById(commentId)
    if (user.email === currentComment.author.email) {
      return this.commentModel.findByIdAndUpdate(commentId, updateCommentDto, { new: true }).exec()
    }
    throw new HttpException('The comment has been updated by the author', HttpStatus.UNAUTHORIZED)
  }
  ...
  async removeCommentById(user: User, commentId: string) {
    ...
    if (user.email === currentComment.author.email) {
      ...
      if (currentComment.comment) {
        ...
        this.postsService.changeNumComments(currentComment.post, numCommentsDelete)
      }
      // delete children comments and this comment
      return this.removeComment(currentComment)
    }
    throw new HttpException('The comment has been deleted by the author', HttpStatus.UNAUTHORIZED)
  }

  async removeCommentsByPostId(user: User, postId: string) {
    try {
      const listComments = await this.getCommentsByPostId(postId)
      if (listComments.length > 0 && user._id.toString() != listComments[0].post.author.toString())
        throw new HttpException('This comments have been deleted by the author of the post', HttpStatus.UNAUTHORIZED)
      for (const eachComment of listComments) {
        this.removeComment(eachComment)
      }
      return HttpStatus.OK
