...
export class TagsService {
    ...
    async addNewQuestionIntoTagsByIds(ids: string[], question: Question): Promise<void> {
        await this.tagModel.updateMany({ _id: { $in: ids } }, { $push: {questionsList: question}}, {multi: true}).exec();
    }

    async deleteQuestionIntoTagsByIds(tags: Tag[], question: Question): Promise<void> {
        await this.tagModel.updateMany({ _id: { $in: tags } }, { $pull: {questionsList: question._id}}, {multi: true}).exec();
    }
